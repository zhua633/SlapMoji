...
export default function EditPageContent() {
  ...
  const generateFramePreview = async (layers: Layer[], canvasWidth: number, canvasHeight: number): Promise<string> => {
  ...
  const calculateBoundingBox = (layers: Layer[], canvasWidth: number, canvasHeight: number) => {
    if (layers.length === 0) {
  ...
  const drawLayersToCanvasTransparent = async (layers: Layer[], canvas: HTMLCanvasElement, offsetX: number = 0, offsetY: number = 0) => {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
  ...
  const drawLayersToCanvas = async (layers: Layer[], canvas: HTMLCanvasElement, offsetX: number = 0, offsetY: number = 0) => {
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
  ...
  // Flip handler
  const handleFlipClick = (e: React.MouseEvent, layer: Layer, direction: 'horizontal' | 'vertical') => {
    e.stopPropagation();
  ...
  const doExport = async (format: "png" | "gif") => {
    ...
    if (format === "gif" && frames.length > 0) {
      ...
      for (let i = 0; i < frames.length; i++) {
        ...
        const bbox = calculateBoundingBox(layersForFrame, editorSize.width, editorSize.height);
      ...
      for (let i = 0; i < frames.length; i++) {
        ...
        if (exportCtx) {
          ...
          exportCtx.drawImage(
            fullCanvas,
            globalMinX, globalMinY, exportWidth, exportHeight,
            0, 0, exportWidth, exportHeight
          );
          gifEncoder.addFrame(exportCtx, { copy: true, delay: frames[i].frame?.delay || 100 });
        }
      }
    ...
    const bbox = calculateBoundingBox(currentLayers, editorSize.width, editorSize.height);
  ...
  const displayLayers = useMemo(() => {
    ...
    if ((isGif(imgUrl || "") || imgType === "image/gif") && frames.length > 0) {
      layers = (frameLayers[safeFrameIdx] || []).map((layer: Layer, idx: number) => {
        if (
          idx === 0 &&
          layer.type === "image" &&
  ...
  useEffect(() => {
    const updatePreviews = async () => {
      ...
      for (let i = 0; i < frameLayers.length; i++) {
        ...
        if (layers && layers.length > 0) {
          const preview = await generateFramePreview(layers, editorSize.width, editorSize.height);
    ...
    if (frameLayers.length > 0 && editorSize.width > 0 && editorSize.height > 0) {
      updatePreviews();
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.key === 'Delete' || e.key === 'Backspace') && selectedLayerId) {
        ...
        setFrameLayers((prev: Layer[][]) => {
          return prev.map((layers, idx) => {
            if (idx !== selectedFrameIdx) return layers;
            const filteredLayers = layers.filter(layer => layer.id !== selectedLayerId);
            ...
            const deletedIndex = layers.findIndex(layer => layer.id === selectedLayerId);
            const newSelectedIndex = Math.max(0, deletedIndex - 1);
            const newSelectedId = filteredLayers[newSelectedIndex]?.id || null;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-black text-white p-8">
      <div className="flex w-full max-w-5xl overflow-hidden gap-6" style={{ height: editorSize.height }}>
